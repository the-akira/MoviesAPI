...
```
http http://mooviesapi.herokuapp.com/api/movies/?search=Space
```
...
```
http https://mooviesapi.herokuapp.com/api/genres/?page=2
```
...
```
http https://mooviesapi.herokuapp.com/api/
```
...
```
http https://mooviesapi.herokuapp.com/api/movies/1/
```
...
You can find Schema for the Movies API by visiting the following link: [Schema API](https://mooviesapi.herokuapp.com/schema)
...
Django Rest framework provides us with an interface that we can [try the API](https://mooviesapi.herokuapp.com/api) without the need for any tool.